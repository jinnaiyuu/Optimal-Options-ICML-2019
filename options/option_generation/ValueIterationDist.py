...
class ValueIterationDist(Planner):

    def __init__(self, mdp, vstar, name="value_iter", delta=0.0001, max_iterations=500, sample_rate=3, epsilon=0.01):
        '''
        Args:
            mdp (MDP)
        ...
        self.fixed_values = defaultdict(float)
        self.vstar = vstar
    ...
    def run_vi(self):
        ...
        while max_diff > self.delta and iterations < self.max_iterations:
            tmp_value_func = defaultdict(float)
            max_diff = 0

            for s in state_space:
                if math.fabs(self.vstar[s] - self.value_func[s]) < self.epsilon:
                    if s not in self.distance.keys():
                        self.distance[s] = iterations
        
            for s in state_space:
                if s in self.fixed_values.keys():
                    continue
                ...
                # Update value.
                tmp_value_func[s] = max_q
            self.value_func = tmp_value_func
            iterations += 1
    ...
    def add_fixed_val(self, state, value):
        self.fixed_values[state] = value
