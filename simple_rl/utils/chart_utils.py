...
def load_data(experiment_dir, experiment_agents):
    ...
    for alg in experiment_agents:
        ...
        for instance in all_reward.readlines():
            ...
            # for r in instance.split(",")[:-1]:
            #     if len(r) > 0:
            #         if float(r) > 0:
            #             all_episodes_for_instance.append(1)
            #         else:
            #             all_episodes_for_instance.append(0)
            # all_episodes_for_instance = [float(i)/len(all_episodes_for_instance) for i in all_episodes_for_instance] 
...
def average_data(data, cumulative=False):
    ...
    for i, all_instances in enumerate(data):
        ...
        for rew in avged:
            total_so_far += rew
            if not cumulative:
                temp.append(total_so_far / len(temp) + 1)
        avged = temp
        result[i] = avged
...
def compute_conf_intervals(data, cumulative=False):
    ...
    for i, all_instances in enumerate(data):
        ...
        for j in range(num_episodes):
            ...
            if not cumulative:
                summed_vector = summed_vector / (j + 1.0)
...
def compute_single_conf_interval(datum):
    ...
    # TODO: Add a function to return the deviation
...
def plot(results, experiment_dir, agents, plot_file_name="", conf_intervals=[], use_cost=False, cumulative=False, episodic=True, open_plot=True, track_disc_reward=False):
    ...
    # ax = pyplot.figure(figsize=(9.5, 6)).gca()
    ...
    for i, agent_name in enumerate(agents):
        ...
        marker_every = max(int(len(y_axis) / 15),1)
...
def main():
    ...
    # Plot.
    make_plots(data_dir, agent_names, cumulative=cumulative, episodic=episodic, track_disc_reward=track_disc_reward, open_plot=False)
