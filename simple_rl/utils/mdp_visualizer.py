...
def visualize(mdp, draw_state, cur_state=None, scr_width=720, scr_height=720, options=[], filename=""):
    '''
    Args:
        mdp (MDP)
        draw_state (lambda: State --> pygame.Rect)
    ...
    # Setup and draw initial state.
    cur_state = mdp.get_init_state() if cur_state is None else cur_state
    agent_shape = _vis_init(screen, mdp, draw_state, cur_state, agent, options=options)
    draw_state(screen, mdp, cur_state, agent=agent, options=options)
    pygame.image.save(screen, filename + '.png')
    
    exit()
        
def _vis_init(screen, mdp, draw_state, cur_state, agent=None, value=False, score=-1, options=[]):
    # Pygame setup.
    pygame.init()
    ...
    agent_shape = draw_state(screen, mdp, cur_state, draw_statics=True, options=options)
