...
class GridWorldMDP(MDP):
    ...
    def bounds(self):
        return (np.asarray([0, 0], dtype=float), np.asarray([self.width, self.height], dtype=float))
    ...
    def visualize(self, options=[], filename=""):
        from simple_rl.utils import mdp_visualizer as mdpv
        from simple_rl.tasks.grid_world.grid_visualizer import _draw_state
        mdpv.visualize(self, _draw_state, options=options, filename=filename)
        input("Press anything to quit")
...
def main():
    ...
    grid_world.visualize_interaction()
