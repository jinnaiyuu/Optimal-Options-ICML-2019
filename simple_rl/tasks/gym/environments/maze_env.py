...
from simple_rl.tasks.gym.environments import maze_env_utils
...
class MazeEnv(gym.Env):
  ...
  def __init__(
    ...
    # Each block is sized size_scaling x size_scaling? (8x8)
    # [1, 1, 1, 1, 1],
    # [1, 'r', 0, 0, 1],
    # [1, 1, 1, 0, 1],
    # [1, 0, 0, 0, 1],
    ...
    # Jinnai: We are changing the simulation setting because the ball actually can
    #         get pass through the obstacles!
    # TODO: We are still getting the ball out of the maze...
  ...
  def step(self, action):
    ...
    # TODO: Why is this set to false??????????
    done = False
    
    # TODO: Let's put the goal further so that we can evaluate its exploration power.
    # goal = np.asarray([16.0, 8.0])
    
    distance_to_goal = np.sum(np.square(next_obs[:2] - self._goal_pos)) ** 0.5

    if self._goal_reward:
      inner_reward = -distance_to_goal / (self.MAZE_SIZE_SCALING * 3.0 * (2.0 ** 0.5))

    if distance_to_goal < 5.0:
      done = True
      if self._goal_reward:
        inner_reward = 10000.0
      
      
      
      # print('state=', next_obs[:2], 'reward=', reward)
