...
class ActionAbstraction(object):
    ...
    def __init__(self, options=None, prim_actions=[], term_prob=0.0, prims_on_failure=False, use_prims=False):
        if use_prims:
            if options is not None:
                self.options = self._convert_to_options(prim_actions) + list(options)
            else:
        ...
        prim_options = self._convert_to_options(prim_actions)
        # print('prim_options=', prim_options)
        # print('set of options=', self.options)
