...
class DQNAgentAtari(Agent):
    ...
    def act(self, state, reward, train=True):
        ...
        '''
        # Training
        if self.should_train and self.total_steps > 0 and self.total_steps % self.update_freq == 0 and train:
            s, a, r, s2, t = self.experience_buffer.sample(self.batch_size)
            self.train_batch(s, a, r, s2, t, self.batch_size)
